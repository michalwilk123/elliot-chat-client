...
from app.chat.crypto.crypto_utils import create_b64_from_public_key, generate_DH
...
def init_mocker(mocker):
    ...
    async def mocked_con_info(contact):
        return {
            ...
            "user_data" : {
                "login": "alice",
                "public_id_key": create_b64_from_public_key(generate_DH().public_key()).decode(PREFFERED_ENCODING),
                "public_signed_pre_key": create_b64_from_public_key(generate_DH().public_key()).decode(PREFFERED_ENCODING),
                "success" : True
            }
        }

    async def mocked_get_otk():
        return create_b64_from_public_key(generate_DH().public_key()).decode(PREFFERED_ENCODING), 1

    async def mocked_friend_request(schema):
        assert set(schema.keys()) == {
            "my_login", "signature", "contact_login", "public_id_key", "public_ephemeral_key", "otk_index"
        }
    
    def mocked_x3dh(*args, **kwargs):
        assert not None in args
        assert not None in kwargs.values()
...
def app_cont_prep(mocker):
    ...
    app_controller.api_controller = ApiController(state, app_controller.db_controller, init_session=False, register_user=False)
